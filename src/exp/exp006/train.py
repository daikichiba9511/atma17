...
class SmoothFocalLoss(nn.Module):
    def __init__(self, reduction="none", alpha=1, gamma=2, smoothing=0.0) -> None:
        super().__init__()
        self.reduction = reduction
...
class CustomTrainer(Trainer):
    ...
    def compute_loss(
        ...
        aux_loss = F.cross_entropy(input=outputs.aux_logits, target=aux_labels, label_smoothing=0.1)
